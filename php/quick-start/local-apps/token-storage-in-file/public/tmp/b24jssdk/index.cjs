...
const $protobuf__default = /*#__PURE__*/_interopDefaultCompat($protobuf);
...
class Http {
  ...
  async batch(calls, isHaltOnError = true) {
    ...
    const processRow = (row, index) => {
      ...
      if (!!method) {
        cnt++;
        let data = method + "?" + qs__default.stringify(params);
      ...
      const initError = (result2) => {
        return new AjaxError({
          ...
          answerError: {
            ...
            errorDescription: `batch ${result2.getQuery().method}: ${qs__default.stringify(result2.getQuery().params, { encode: false })}`
          },
  ...
  async call(method, params, start = 0) {
    ...
    return this.#clientAxios.post(
      ...
      async (problem) => {
        ...
        if (problem instanceof axios.AxiosError && problem.response && problem.response.data) {
          ...
          if ([
            ...
            return this.#clientAxios.post(
              ...
              async (problem2) => {
                ...
                if (problem2 instanceof axios.AxiosError && problem2.response && problem2.response.data) {
...
let $Reader = $protobuf__default.Reader, $Writer = $protobuf__default.Writer, $util = $protobuf__default.util;
const $root = $protobuf__default.roots["push-server"] || ($protobuf__default.roots["push-server"] = {});
