...
## Обязательное завершение задачи

После завершения любой задачи нужно обязательно прогонять unit-тесты через `make test`.

- Это правило действует даже для небольших локальных изменений, если в репозитории есть тесты.
- Если тесты не запускаются или падают, это нужно явно указать в итоговом сообщении.
- Не заменяй этот шаг прямым вызовом `vendor/bin/phpunit`: используй только `make test`.
