...
## Bounded context
Бизнес работает с разными категориями клиентов. В зависимости от категории клиенты могут быть надежными или рискованными.
Численную оценку уровня риска делает внешняя система и возвращает в виде числа в диапазоне от 1 до 20.
Для удобства работы введен атрибут: «Уровень риска». Возможные уровни риска:
- низкий <= 5
- средний от 6 до 9 
- высокий >= 10

Правила оценки уровня могут со временем меняться, историю перехода между уровнями хранить не требуется.

### Отражение в коде
Объект Score c инвариантами
- диапазон оценки от 1 до 20
- соответствие оценки и уровня риска 

### Отражение в Битрикс24
Уровни риска хранятся в смарт-процессе. Для каждого уровня указано название и его символьный код. Привязка уровня риска осуществляется к контакту.
В контакте создано пользовательское поле, тип integer в котором хранится текущая оценка риска для контакта.
