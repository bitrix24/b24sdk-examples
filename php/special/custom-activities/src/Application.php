...
class Application
{
    ...
    public static function processInstallation(Request $incomingRequest)
    {
        ...
        if (PlacementRequest::isCanProcess($incomingRequest)) {
            ...
            self::getB24Service($placementRequest->getRequest())
                // unbind if reinstall
                ->getPlacementScope()->placement()->unbind(PlacementLocationCode::CRM_DEAL_DETAIL_ACTIVITY);
            
            // bind placement handler
            $regResult = self::getB24Service($placementRequest->getRequest())
            ...
            self::getLog()->debug('processRequest.processOnInstallPlacementRequest.finishBindPlacements', [
                'result' => $regResult->getCoreResponse()
            ]);
    ...
    public static function getB24Service(?Request $request = null): ServiceBuilder
    {
        ...
        if ($request->request->get('action') !== null) {
            ...
            self::getLog()->debug('getB24Service.createByRequest');

            // some actions for preparing auth data from request
            
            $auth = new LocalAppAuth(
                new AuthToken(
                    $request->request->get('access_token'), 
                    $request->request->get('refresh_token'),
                    intval($request->request->get('expires')),
                    intval($request->request->get('expires_in'))
                ), 
                $request->request->get('domain'), null
            );
