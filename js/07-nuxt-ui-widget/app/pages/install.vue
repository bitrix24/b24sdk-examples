<script setup lang="ts">
import { ref, onMounted, onUnmounted } from 'vue'
import Logo from '~/components/Logo.vue'

export interface IStep {
    action: () => Promise<void>
    caption?: string
    data?: Record<string, any>
}

definePageMeta({
    layout: 'clear'
})

useHead({
    title: 'Install',
})

// region Init ////
const confetti = useConfetti()
const router = useRouter()

const isInit = ref(false)
const isShowDebug = ref(false)

const progressColor = ref<string>('primary' as const)
const progressValue = ref<null | number>(null)

const globalStore = useGlobalStore()
// endregion ////

// region Steps ////
const steps = ref<Record<string, IStep>>({
    init: {
        caption: 'Initialization',
        action: makeInit
    },
    placement: {
        caption: 'Widget Installation',
        action: async () => {
            /**
             * Registering placement
             */
            if (steps.value.placement) {

                let result = await globalStore.b24?.callMethod('placement.unbind',
                    {
                        "PLACEMENT": "PAGE_BACKGROUND_WORKER",
                    })
                
                result = await globalStore.b24?.callMethod('placement.bind',
                    {
                        "PLACEMENT": "PAGE_BACKGROUND_WORKER",
                        "HANDLER": "https://whale-viable-wasp.ngrok-free.app/background",
                        "TITLE": "Background worker",
                        "OPTIONS": {
                            "errorHandlerUrl": "https://whale-viable-wasp.ngrok-free.app/",
                        }
                    })

                if (result?.hasError) {
                    console.error('Error:', result.getErrorMessages())
                } else {
                    console.log('Success:', result)
                }

            }

            return sleepAction()
        }
    },
    finish: {
        caption: 'Installation Complete',
        action: makeFinish
    }
})
const stepCode = ref<string>('init' as const)
// endregion ////

// region Actions ////
async function makeInit(): Promise<void> {
    // const { $initializeB24Frame } = useNuxtApp()
    // $b24 = await $initializeB24Frame()
    // $b24.setLogger(LoggerBrowser.build('Core'))

    isInit.value = true

    /**
     * @todo add lang
     */

    // await $b24.parent.setTitle('App: install')

    return sleepAction()
}

async function makeFinish(): Promise<void> {
    progressColor.value = 'success' as const
    progressValue.value = 100

    confetti.fire()
    await sleepAction(2000)

    if (globalStore.b24) {
        // globalStore.b24.installFinish()
    }
    // await router.push('/')
}

const stepsData = computed(() => {
    return Object.entries(steps.value).map(([index, row]) => {
        return {
            step: index,
            data: row?.data
        }
    })
})
// endregion ////

async function sleepAction(timeout: number = 1000): Promise<void> {
    return new Promise<void>(resolve => setTimeout(resolve, timeout))
}

// region Mounted / Unmounted ////
onMounted(async () => {
    try {
        for (const [key, step] of Object.entries(steps.value)) {
            stepCode.value = key
            await step.action()
        }
    } catch (error: any) {
        /**
         * @todo fix error
         */
        // $logger.error(error)
        showError({
            statusCode: 404,
            statusMessage: error?.message || error,
            data: {
                description: error?.message || error,
                homePageIsHide: true,
                isShowClearError: true,
                clearErrorHref: '/'
            },
            cause: error,
            fatal: true
        })
    }
})

onUnmounted(() => {
    // $b24?.destroy()
})
// endregion ////
</script>

<template>
    <div class="mx-3 flex flex-col items-center justify-center gap-1 h-dvh">
        <Logo class="size-52" :class="[
            stepCode === 'finish' ? 'text-green-500' : 'text-base-500'
        ]" />
        <B24Progress v-model="progressValue" size="xs" animation="elastic" color="primary" class="w-1/2 sm:w-1/3" />
        <div class="mt-6 flex flex-col items-center justify-center gap-2">
            <div class="text-h1 text-nowrap">
                Installation
            </div>
            <div class="text-base-500">
                {{ steps[stepCode]?.caption || '...' }}
            </div>
        </div>

        <ProsePre v-if="isShowDebug">
            {{ stepsData }}
        </ProsePre>
    </div>
</template>